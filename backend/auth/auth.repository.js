...
const getLastSession = async (user_id) => {
    const [rows] = await db.query(
        'SELECT user_id, device_name, browser, os, ip_address, country, user_agent, last_active_at FROM sessions WHERE user_id = ?  ORDER BY last_active_at DESC LIMIT 1',
        [user_id]
    );
    return rows[0];
};
